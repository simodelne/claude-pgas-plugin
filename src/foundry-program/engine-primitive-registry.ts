...
export const ENGINE_DECLARATION_AWARENESS: readonly EngineDeclarationAwarenessEntry[] = [
  ...
  { construct: 'EngineCapability.render', status: 'adopt_backlog', note: 'v5.4.0/5.6.0 #992 render capability + engine-native ArtifactStore sink (PARTIAL). Foundry already emits a RenderProfile for the PDF report (buildPdfReportRenderProfile) but currently set-but-inert as a byte source (no capability: render channel / renderProvider wired). PARTIAL 0-TS: the docx/html shape-mapping stage-body TS (renderDocxExportStageBody/sectionsFromDomain) plus the base64-in-domain+artifactPolicy persistence seam ARE subsumable by declarative render: + capability: render + engine-native ArtifactStore; the OOXML byte serializer stays consumer-owned by design (engine never bundles a renderer, node_modules/@simodelne/pgas-server/dist-bundle/_shared-types.d.ts:7897-7899) collapsing to ONE generic RenderProvider. MECHANISM PROVEN hermetically (tests/integration/render-capability-falsifier.test.ts on feat/v56-render-capability: hand-authored render: program -> provider_request projection -> generic RenderProvider (templates/pgas-new/consumer/render-provider.ts.tmpl, a pure IR->OOXML serializer) -> engine ArtifactStore.put -> first-class artifactType:"render" record, nonce verbatim, kill-mutation proven). But SYNTHESIS EMISSION IS BLOCKED: the foundry has NO fixed-shape docx to migrate — its entire docx surface is the dynamic per-approved-item class (approvedContentSectionsFromDomain, N runtime-determined multi-paragraph sections), which the render: grammar cannot express (only collection-iterating node RenderTable.rows flattens to scalar cells). Deleting the imperative TS would regress every docx deliverable; forcing a table would corrupt it. Blocked on the RenderSectionList repeat-node primitive = simodelne/pgas#1045 (gap docs/curator-requests/2026-08-23-render-section-list-primitive.md; additive -> v5.7.0). Until #1045 ships, EngineCapability.render stays adopt_backlog and the foundry keeps emitting renderDocxExportStageBody/sectionsFromDomain. Anchors node_modules/@simodelne/pgas-server/dist-bundle/_shared-types.d.ts:5830,6096-6109,6248-6254,6576-6577,7840-7844,7897-7901,8235.' },
