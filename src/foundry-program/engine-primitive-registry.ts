...
export const ENGINE_DECLARATION_AWARENESS: readonly EngineDeclarationAwarenessEntry[] = [
  ...
  { construct: 'EngineCapability.render', status: 'adopt_backlog', note: 'v5.4.0/5.6.0 #992 render capability + engine-native ArtifactStore sink. GRAMMAR GAP CLOSED: the RenderSectionList repeat-node asked for in docs/curator-requests/2026-08-23-render-section-list-primitive.md (simodelne/pgas#1045) SHIPPED in 5.7.1 (RenderSectionList at node_modules/@simodelne/pgas-server/dist-bundle/_shared-types.d.ts:5511-5517, admitted at RenderArtifact.sections:5590). The foundry\'s per-approved-item docx class is now expressible declaratively and is PROVEN END-TO-END HERMETICALLY on 5.7.1 in tests/integration/render-section-list-falsifier.test.ts: program-owned approval -> the engine\'s own derived_paths[items_where_field_eq] approved bucket -> a TOP-LEVEL render: section_list over that bucket -> buildProviderRenderRequest -> the ONE generic consumer RenderProvider (templates/pgas-new/consumer/render-provider.ts.tmpl, a pure IR->OOXML serializer) -> engine ArtifactStore.put -> first-class artifactType:"render" record; S-1 renders one docx section per approved item with the non-approved item absent, and both kill-proofs were OBSERVED failing under sabotage (K-1 rebinding `from` makes the approved prose vanish; K-2 an empty bucket renders ZERO sections and fabricates nothing). EMISSION IS STILL BLOCKED, now on a DIFFERENT and narrower gap: DISPATCH from an AUTHOR-LESS stage. The foundry\'s export stage is a decision_only mode (pgas-new #253; the author is never called for it, asserted by tests/integration/export-decision-only-autoadvance-falsifier.test.ts), so it can emit no EffectAction and its only outbound seam is an OnTransition integration hook. capability: render fires its declarative projection only for a payload whose own keys are exactly the {artifact_id} selector, while the engine\'s hook envelope is {action, event, domain} — so the projection is skipped and the raw envelope reaches the RenderProvider (OBSERVED: capability.render.error "RenderProvider: request.format must be \\"docx\\"", 0 artifacts, session Failed; pinned by G-1 in the same falsifier). IntegrationHook has no payload field. Ask filed at docs/curator-requests/2026-08-24-declarative-render-dispatch-author-less-stage.md. Until that ships, EngineCapability.render stays adopt_backlog and the foundry keeps emitting renderDocxExportStageBody/renderHtmlExportStageBody/approvedContentSectionsFromDomain plus the base64-in-domain+artifactPolicy persistence seam; making the export stage LLM-driven instead would regress the author-less property and require loosening a shipped falsifier, and reshaping the hook envelope in consumer code would be the forbidden stopgap. The OOXML byte serializer stays consumer-owned by design (the engine never bundles a renderer) collapsing to ONE generic RenderProvider. Anchors node_modules/@simodelne/pgas-server/dist-bundle/_shared-types.d.ts:5511-5517,5590,5711-5720,8564-8589 and the renderCapability exact-selector + dispatchHook envelope in dist-bundle/plugin.mjs.' },
