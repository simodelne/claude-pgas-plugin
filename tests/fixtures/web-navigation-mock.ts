...
export class MockWebNavigationConnector implements WebNavigationHostConnector {
  async navigate_and_extract(
    ...
    const pages = fixturePagesForSource(source).slice(0, Math.max(0, guard.max_pages));
    for (const page of pages) {
      audit.push({ action: 'fetch', url: page.url, at_depth: page.depth });
    }

    const items = Object.keys(extraction_schema).map((key, index) => {
...
function fixturePagesForSource(source: string): Array<{ url: string; depth: number }> {
  const fixture = [
    { url: source, depth: 0 },
    { url: 'https://example.com/team/alex', depth: 1 },
    { url: 'https://example.com/team/blair', depth: 1 },
  ];
  return source.includes('example.com') ? fixture : [{ url: source, depth: 0 }];
}
